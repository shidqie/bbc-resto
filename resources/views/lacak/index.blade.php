    ...
    <section class="py-16 bg-canvas min-h-screen">
        <div class="max-w-5xl mx-auto px-4 lg:px-8">
            ...
            <form method="GET" action="{{ route('lacak.index') }}" class="mb-8">
                <div class="flex gap-3">
                    <input
                        ...
                        class="flex-1 border border-gray-200 rounded-3xl px-4 py-3 text-sm focus:ring-2 focus:ring-emerald-500 focus:border-transparent outline-none shadow-sm"
                        required
                    >
                    <button type="submit" class="bg-[#0F2E23] hover:bg-[#0a1f17] text-white font-bold px-6 py-3 rounded-3xl text-sm transition-all shadow-sm flex items-center gap-2">
            ...
            @if($kodePesanan && !$pesanan)
                <div class="bg-red-50 border border-red-200 rounded-[2.25rem] p-6 text-center">
            ...
            <div class="mb-12">
                ...
                <div class="grid grid-cols-1 lg:grid-cols-12 gap-10 lg:gap-14 items-start">
                    ...
                    <div class="lg:col-span-6 space-y-8">
                        <div>
                            ...
                            <div class="space-y-4 text-sm text-gray-600">
                                ...
                                <div class="flex justify-between items-center py-2.5 border-b border-gray-100/60">
                                    ...
                                    <span class="font-medium text-gray-800">{{ $pesanan->jadwal_pesanan->nomor_telepon_penerima ?? '-' }}</span>
                                </div>
                                <div class="flex justify-between items-center py-2.5 border-b border-gray-100/60">
                        ...
                        {{-- Total & Payment Box --}}
                        <div class="bg-gray-50/70 border border-gray-100 rounded-[2.25rem] p-5 space-y-3">
                        ...
                        <div class="space-y-3 pt-2">
                            @if(in_array($statusBayarKey, ['belum_bayar', 'dp_terbayar']) && $pesanan->status_pesanan_id != 6)
                                <a href="{{ route('pesanan.bayar', $pesanan->nomor_pesanan) }}" class="w-full flex items-center justify-center gap-2 bg-[#3B82F6] hover:bg-blue-600 text-white font-bold py-4 px-6 rounded-[2.25rem] text-xs tracking-widest uppercase transition-all shadow-md shadow-blue-500/20 active:scale-[0.99]">
                            ...
                            @if($jenisPesanan !== 'Dine In / Takeaway')
                                <a href="{{ route('pesanan.invoice', $pesanan->nomor_pesanan) }}" target="_blank" class="w-full flex items-center justify-center gap-2 bg-emerald-50 hover:bg-emerald-100 text-emerald-700 border border-emerald-200/60 font-bold py-3.5 px-6 rounded-[2.25rem] text-xs tracking-widest uppercase transition-all">
                                    <i class="ph-bold ph-download-simple text-base"></i>
                                    <span>Unduh Bukti Pesanan</span>
                                </a>
                            @endif

                            <a href="{{ route('lacak.index') }}" class="w-full flex items-center justify-center gap-2 bg-gray-50 hover:bg-gray-100 text-gray-500 hover:text-gray-700 font-bold py-3.5 px-6 rounded-[2.25rem] text-xs tracking-widest uppercase transition-all">
