...
<div x-data="{ open: {{ $isOpen ? 'true' : 'false' }}, hover: false }" class="relative" @mouseenter="hover = true" @mouseleave="hover = false">
    ...
    <button @click="if(!sidebarOpen) { sidebarOpen = true; open = true; } else { open = !open; }"
            class="flex items-center justify-between w-full px-3 py-2.5 rounded-lg text-base transition-colors duration-200 focus:outline-none group relative"
            :class="open ? 'bg-neutral-100/80 text-[#0D3024] font-semibold' : 'text-neutral-500 hover:text-neutral-900 hover:bg-neutral-50'"
        ...
        <div class="flex items-center gap-3 flex-1 overflow-hidden">
            <x-dynamic-component :component="$icon"
               ...
               x-bind:class="open ? 'text-[#0D3024]' : 'text-neutral-400 group-hover:text-neutral-600'" />
        ...
        <x-heroicon-s-chevron-down x-show="sidebarOpen"
           ...
           x-bind:class="open ? 'rotate-180 text-neutral-900' : 'text-neutral-400 group-hover:text-neutral-600'" />
    </button>

    {{-- Dropdown (expanded sidebar) --}}
    <div x-show="open && sidebarOpen" x-collapse class="mt-1 space-y-1" style="display: none;">
        @foreach($items as $item)
            <a href="{{ $item['url'] }}"
               class="flex items-center gap-2.5 ml-[2.35rem] pr-3 py-1.5 rounded-lg text-sm transition-colors duration-150 group/child
                      {{ $item['active']
                          ? 'text-[#0D3024] font-semibold'
                          : 'text-neutral-500 hover:text-neutral-900 hover:bg-neutral-50' }}">
                <span class="w-1.5 h-1.5 rounded-full shrink-0 transition-colors {{ $item['active'] ? 'bg-[#0D3024]' : 'bg-transparent border border-neutral-300 group-hover/child:border-neutral-500' }}"></span>
                {{ $item['label'] }}
            </a>
        @endforeach
    </div>
            ...
            @foreach($items as $item)
                <a href="{{ $item['url'] }}"
                   class="block px-3 py-2 rounded-lg text-sm transition-colors duration-150
                          {{ $item['active']
                              ? 'bg-neutral-100/80 text-[#0D3024] font-medium'
