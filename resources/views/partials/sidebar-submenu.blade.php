...
<div x-data="{ open: {{ $isOpen ? 'true' : 'false' }}, hover: false }" class="relative" @mouseenter="hover = true" @mouseleave="hover = false">
    ...
    <button @click="if(!sidebarOpen) { sidebarOpen = true; open = true; } else { open = !open; }"
            ...
            :class="[
                open ? 'bg-primary-soft text-primary font-semibold shadow-sm' : 'text-slate-600 font-medium hover:text-slate-900 hover:bg-slate-100/80',
        ...
        <div class="flex items-center overflow-hidden" :class="sidebarOpen ? 'gap-3.5 flex-1' : ''">
            <x-dynamic-component :component="$icon"
               ...
               x-bind:class="open ? 'text-primary' : 'text-slate-400 group-hover:text-slate-600'" />
        ...
        <x-heroicon-s-chevron-down x-show="sidebarOpen"
           ...
           x-bind:class="open ? 'rotate-180 text-primary' : 'text-slate-400 group-hover:text-slate-500'" />
    </button>
    ...
    <div x-show="open && sidebarOpen" x-collapse class="mt-1 ml-[1.65rem] border-l-2 border-slate-100 py-1" style="display: none;">
        @foreach($items as $item)
            <a href="{{ $item['url'] }}"
               class="flex items-center ml-2 px-3 py-2 text-sm rounded-xl transition-all duration-200 group/child relative
                      {{ $item['active']
                          ? 'text-primary bg-primary/20 font-semibold shadow-sm'
                          : 'text-slate-500 font-medium hover:text-slate-900 hover:bg-slate-50' }}">
                
                @if($item['active'])
                    <span class="absolute -left-[10px] top-0 bottom-0 w-[2px] bg-primary rounded-full"></span>
                @else
    ...
    <div x-show="hover && !sidebarOpen" x-cloak
        ...
        <div class="px-3 py-2 text-xs font-bold text-primary uppercase tracking-wider border-b border-gray-50 flex items-center gap-2">
            <span>{{ $label }}</span>
        </div>
