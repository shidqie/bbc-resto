...
<head>
    <meta charset="UTF-8">
    <title>Rincian Pesanan Katering #{{ $pesanan->kode_pesanan }}</title>
    <style>
        @page {
            size: A4 portrait;
...
<body>
    ...
    <table class="header-table">
        <tr>
            <td style="vertical-align: top;">
                <h1 class="header-title">Rincian Pesanan Katering (Internal)</h1>
    ...
    <table class="info-table">
        ...
        <tr>
            <td class="info-label">Lokasi Acara</td>
            <td class="info-colon">:</td>
            <td class="info-val">{{ $pesanan->lokasi_acara }}</td>
        </tr>
        <tr>
            <td class="info-label">Jumlah Porsi</td>
            <td class="info-colon">:</td>
            <td class="info-val">{{ $pesanan->jumlah_porsi }} Porsi</td>
        </tr>
        <tr>
    ...
    <div style="margin-top: 8px; font-size: 11px;">
        ...
        <ul style="margin: 0 0 12px 15px; padding: 0;">
            ...
            <li style="margin-bottom: 3px;">
                {{ $detail->komponen->nama_komponen ?? '-' }}: <strong>{{ $detail->menu->nama_menu ?? '-' }}</strong>
            </li>
            @endforeach
        </ul>
    </div>
    ...
    <ul style="margin: 8px 0 12px 15px; padding: 0; font-size: 11px;">
        @foreach($pesanan->addons as $addon)
        <li style="margin-bottom: 3px;">
            {{ $addon->layananTambahan->nama_layanan ?? '-' }} (<strong>{{ $addon->qty }}</strong>)
        </li>
        @endforeach
    </ul>
    @endif
    ...
    <table class="notes-table">
        ...
        <tr>
            <td>1. Pastikan jumlah porsi dan pesanan sudah sesuai sebelum diantarkan.</td>
        </tr>
        <tr>
