            ...
            <form action="{{ route('menu.update', $menu->id) }}" method="POST" enctype="multipart/form-data" class="p-6 space-y-6">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    ...
                    <div class="md:col-span-2">
                        ...
                        <input type="text" name="nama" value="{{ old('nama', $menu->nama) }}" required class="w-full px-4 py-2 border border-gray-200 rounded-xl focus:ring-2 focus:ring-blue-100 focus:border-[#3B82F6] outline-none transition-all">
                    </div>
                    ...
                    <div>
                        ...
                        <select name="kategori_menu_id" required class="w-full px-4 py-2 border border-gray-200 rounded-xl focus:ring-2 focus:ring-blue-100 focus:border-[#3B82F6] outline-none bg-white transition-all">
                            ...
                            @foreach($kategoris as $kategori)
                                <option value="{{ $kategori->id }}" {{ old('kategori_menu_id', $menu->kategori_menu_id) == $kategori->id ? 'selected' : '' }}>{{ $kategori->nama_kategori ?? $kategori->nama }}</option>
                            @endforeach
                        </select>
                    </div>
                    ...
                    <div>
                        ...
                        <select name="jenis_menu" required class="w-full px-4 py-2 border border-gray-200 rounded-xl focus:ring-2 focus:ring-blue-100 focus:border-[#3B82F6] outline-none bg-white transition-all">
                            <option value="dine_in" {{ old('jenis_menu', $menu->jenis_menu) == 'dine_in' ? 'selected' : '' }}>Dine-in (Makan di tempat)</option>
                            <option value="catering" {{ old('jenis_menu', $menu->jenis_menu) == 'catering' ? 'selected' : '' }}>Catering</option>
                            <option value="nasi_box" {{ old('jenis_menu', $menu->jenis_menu) == 'nasi_box' ? 'selected' : '' }}>Nasi Box</option>
                        </select>
                    </div>
                    ...
                    <div>
                        ...
                        <input type="number" name="harga" value="{{ old('harga', $menu->harga) }}" required min="0" step="1" class="w-full px-4 py-2 border border-gray-200 rounded-xl focus:ring-2 focus:ring-blue-100 focus:border-[#3B82F6] outline-none transition-all">
                    </div>
                ...
                <div class="flex justify-end gap-3 pt-6 border-t border-gray-100">
                    ...
                    <x-ui.button type="submit" icon="fa-save">Simpan Perubahan</x-ui.button>
                </div>
            </form>
        </div>
    </div>
