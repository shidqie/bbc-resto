...
<div class="flex-1 overflow-auto bg-gray-50/50 text-gray-800 font-sans">
    <div class="w-full p-6 ] space-y-6">
        ...
        <div class="bg-white rounded-[2.25rem] border border-gray-200/80 shadow-2xs p-6 space-y-6">
            <form action="{{ route('paket-catering.update', $paketCatering->id) }}" method="POST" enctype="multipart/form-data" id="paketForm" class="space-y-6">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                        <div>
                            ...
                            <input type="text" name="nama_paket" required value="{{ old('nama_paket', $paketCatering->nama_menu) }}" 
                                   class="w-full px-3.5 py-2.5 bg-gray-50 border border-gray-200 rounded-3xl focus:ring-2 focus:ring-[#0F2E23]/10 focus:border-[#0F2E23] outline-none text-xs font-extrabold text-gray-900">
                        </div>
                        ...
                        <div>
                            ...
                            <select name="jenis_paket" required class="w-full px-3.5 py-2.5 bg-gray-50 border border-gray-200 rounded-3xl focus:ring-2 focus:ring-[#0F2E23]/10 focus:border-[#0F2E23] outline-none text-xs font-bold text-gray-900">
                        ...
                        <div>
                            ...
                            <input type="number" name="harga" required min="0" value="{{ old('harga', $paketCatering->harga_jual) }}" 
                                   class="w-full px-3.5 py-2.5 bg-gray-50 border border-gray-200 rounded-3xl focus:ring-2 focus:ring-[#0F2E23]/10 focus:border-[#0F2E23] outline-none text-xs font-black text-[#0F2E23]">
                        </div>

                        <div>
                            <label class="block text-xs font-bold text-gray-700 uppercase tracking-wide mb-1">Foto Paket</label>
                            <input type="file" name="foto" accept="image/*"
                                   class="w-full px-3 py-2 bg-gray-50 border border-gray-200 rounded-3xl text-xs font-medium text-gray-700">
                        ...
                        <div class="md:col-span-2">
                            ...
                            <input type="text" name="deskripsi" value="{{ old('deskripsi', $paketCatering->deskripsi) }}" placeholder="Contoh: Paket hemat prasmanan lengkap…" 
                                   class="w-full px-3.5 py-2.5 bg-gray-50 border border-gray-200 rounded-3xl focus:ring-2 focus:ring-[#0F2E23]/10 focus:border-[#0F2E23] outline-none text-xs font-medium">
                        </div>
                    </div>
                </div>
                ...
                <div class="space-y-4 pt-2">
                    <div class="flex justify-between items-center border-b border-gray-100 pb-2">
                        ...
                        <button type="button" onclick="addKomponen()" class="px-4 py-2 bg-emerald-50 hover:bg-emerald-100 text-[#0F2E23] border border-emerald-200 rounded-3xl text-xs font-bold transition-all flex items-center gap-1.5 shadow-2xs">
                ...
                <div class="pt-4 border-t border-gray-100 flex justify-end gap-2">
                    <a href="{{ route('paket-catering.index', ['jenis' => $jenis]) }}" class="px-5 py-2.5 rounded-3xl border border-gray-200 text-xs font-bold text-gray-600 hover:bg-gray-50 transition-all">
                        Batal
                    </a>
                    <button type="submit" class="px-6 py-2.5 rounded-3xl bg-[#0F2E23] hover:bg-[#0a1f17] text-white text-xs font-extrabold transition-all shadow-sm active:scale-95 flex items-center gap-2">
...
<script>
    ...
    function addKomponen(data = null) {
        ...
        const html = `
        <div class="komponen-card bg-gray-50/80 border border-gray-200/90 p-4 rounded-[2.25rem] relative shadow-2xs space-y-3" id="komp_${currentKompIndex}">
            <button type="button" onclick="document.getElementById('komp_${currentKompIndex}').remove()" class="absolute top-3.5 right-3.5 text-red-500 hover:text-red-700 bg-red-50 hover:bg-red-100 p-1.5 rounded-3xl transition-colors" title="Hapus Komponen">
            ...
            <div class="grid grid-cols-1 md:grid-cols-3 gap-3 pr-10">
                <div>
                    ...
                    <input type="text" name="komponen[${currentKompIndex}][nama_komponen]" required value="${namaVal}" placeholder="Cth: Aneka sup / Sayuran" class="w-full text-xs font-bold px-3.5 py-2 border border-gray-200 bg-white rounded-3xl focus:border-[#0F2E23] outline-none">
                </div>
                <div>
                    <label class="block text-[11px] font-bold text-gray-600 uppercase tracking-wide mb-1">Tipe Pilihan</label>
                    <select name="komponen[${currentKompIndex}][tipe]" required class="w-full text-xs font-bold px-3.5 py-2 border border-gray-200 bg-white rounded-3xl focus:border-[#0F2E23] outline-none">
                ...
                <div>
                    ...
                    <input type="number" name="komponen[${currentKompIndex}][urutan]" required value="${urutanVal}" class="w-full text-xs font-bold px-3.5 py-2 border border-gray-200 bg-white rounded-3xl focus:border-[#0F2E23] outline-none">
                </div>
            </div>
            
            <div class="flex flex-col mb-2.5">
                <label class="text-[11px] font-extrabold text-gray-700 uppercase tracking-wide mb-1">Pilihan Menu (Jika tipe pilihan):</label>
                <p class="text-[10px] text-gray-500 mb-2">Pisahkan dengan koma, contoh: Sup Kimlo, Sup Bakso, Sup Ayam Sosis</p>
                <input type="text" name="komponen[${currentKompIndex}][pilihan]" value="${pilihanText}" placeholder="Cth: Nasi Goreng, Mie Goreng" class="px-3.5 py-2 bg-white border border-gray-200 rounded-3xl text-xs outline-none w-full focus:border-[#0F2E23]">
            </div>
        </div>
        `;
