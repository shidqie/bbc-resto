...
<div class="flex-1 overflow-auto bg-gray-50/50 text-gray-800 font-sans">
    <div class="w-full p-6 ] space-y-6">
        ...
        <x-ui.page-header 
            ...
            <x-slot:actions>
                <x-ui.button href="{{ route('paket-catering.index', ['jenis' => $paketCatering->jenis_paket]) }}" variant="outline" icon="arrow-left">Kembali</x-ui.button>
            </x-slot:actions>
        </x-ui.page-header>
    ...
    function addKomponen(data = null) {
        ...
        <div class="komponen-card bg-gray-50/80 border border-gray-200/90 p-4 rounded-2xl relative shadow-2xs space-y-3" id="komp_${currentKompIndex}">
            ...
            <div class="flex flex-col mb-2.5">
                ...
                <input type="text" name="komponen[${currentKompIndex}][pilihan]" value="${pilihanText}" placeholder="Cth: Nasi Goreng, Mie Goreng" class="px-3.5 py-2 bg-white border border-gray-200 rounded-xl text-xs outline-none w-full focus:border-[#0F2E23]">
            </div>
        </div>
        `;
