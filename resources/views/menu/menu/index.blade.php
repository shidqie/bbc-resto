    ...
    <div class="w-full p-6 space-y-5">
        ...
        <div class="flex flex-col sm:flex-row gap-2 items-start sm:items-center justify-between mb-3 shrink-0">
            ...
            <div class="flex items-center gap-1 text-xs font-medium overflow-x-auto no-scrollbar shrink-0">
                ...
                <a href="{{ route('menu.index') }}" class="px-3 py-1.5 rounded-lg transition-colors {{ !request('jenis_menu_id') ? 'bg-gray-900 text-white' : 'text-gray-600 hover:bg-gray-100' }}">Semua</a>
                <a href="{{ route('menu.index', ['jenis_menu_id' => 'dine_in']) }}" class="px-3 py-1.5 rounded-lg transition-colors {{ request('jenis_menu_id') === 'dine_in' ? 'bg-gray-900 text-white' : 'text-gray-600 hover:bg-gray-100' }}">Resto ({{ $stats['dine_in'] }})</a>
                <a href="{{ route('menu.index', ['jenis_menu_id' => 'catering']) }}" class="px-3 py-1.5 rounded-lg transition-colors {{ request('jenis_menu_id') === 'catering' ? 'bg-gray-900 text-white' : 'text-gray-600 hover:bg-gray-100' }}">Catering ({{ $stats['catering'] }})</a>
                <a href="{{ route('menu.index', ['jenis_menu_id' => 'nasi_box']) }}" class="px-3 py-1.5 rounded-lg transition-colors {{ request('jenis_menu_id') === 'nasi_box' ? 'bg-gray-900 text-white' : 'text-gray-600 hover:bg-gray-100' }}">Nasi Box ({{ $stats['nasi_box'] }})</a>
            </div>
        </div>
            {{-- Menu Table --}}
                    ...
                    <tbody class="divide-y divide-gray-50">
                        ...
                        <tr class="hover:bg-gray-50/60 transition-colors group">
                            ...
                            <td class="px-4 py-3 text-sm text-gray-700">
                                @php
                                    $jName = $menu->jenis_menu->nama_jenis ?? (['1' => 'Menu Reguler', '2' => 'Catering', '3' => 'Nasi Box', 'dine_in' => 'Menu Reguler', 'catering' => 'Catering', 'nasi_box' => 'Nasi Box'][$menu->jenis_menu_id] ?? 'Menu Reguler');
                                @endphp
                                {{ $jName }}
                            </td>
                            <td class="px-4 py-3 text-sm text-gray-700">
                            ...
                            <td class="px-4 py-3 text-center text-sm font-medium">
                                <div class="flex items-center justify-center gap-1.5">
                                    <button onclick="openMenuModal({{ $menu->id }}, true)" title="Detail" class="w-7 h-7 rounded-lg flex items-center justify-center bg-blue-50 text-blue-600 hover:bg-blue-100 transition-colors">
                                        <x-heroicon-o-eye class="w-3 h-3" />
                                    </button>
                                    <button onclick="openMenuModal({{ $menu->id }}, false)" title="Ubah" class="w-7 h-7 rounded-lg flex items-center justify-center bg-amber-50 text-amber-600 hover:bg-amber-100 transition-colors">
                                        <x-heroicon-o-pencil-square class="w-3 h-3" />
                                    </button>
                                    <button onclick="deleteMenu({{ $menu->id }})" title="Hapus" class="w-7 h-7 rounded-lg flex items-center justify-center bg-red-50 text-red-600 hover:bg-red-100 transition-colors">
