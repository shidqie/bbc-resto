    ...
    <div class="w-full p-6 space-y-5">
        ...
        <div class="flex items-center justify-between gap-3">
            ...
            <div class="flex items-center gap-2">
                <button onclick="openModal('modalTambah', 'drawerTambahPanel')" class="inline-flex items-center gap-1.5 text-sm font-semibold text-white bg-gray-900 rounded-2xl px-3 py-2 hover:bg-gray-800 transition-colors">
        ...
        <div class="flex flex-col sm:flex-row gap-2 items-start sm:items-center justify-between mb-3 shrink-0">
            <form action="{{ route('kategori-menu.index') }}" method="GET" class="flex items-center gap-2 w-full sm:w-auto">
                <div class="relative flex-1 sm:flex-none sm:w-56">
                    ...
                    <input type="text" name="search" value="{{ request('search') }}" placeholder="Cari nama kategori…" class="w-full pl-9 pr-3 py-2 text-xs border border-gray-200 rounded-2xl focus:ring-1 focus:ring-gray-400 focus:border-gray-400 outline-none bg-white">
                </div>
...
<div id="modalTambah" class="fixed inset-x-0 bottom-0 top-16 z-50 hidden">
    ...
    <div class="absolute right-0 top-0 h-full w-full max-w-md bg-white shadow-2xl flex flex-col translate-x-full transition-transform duration-300" id="drawerTambahPanel">
        <div class="flex items-center justify-between px-5 py-4 border-b border-gray-100 shrink-0">
            ...
            <button onclick="closeModal('modalTambah', 'drawerTambahPanel')" class="p-1.5 rounded-2xl text-gray-400 hover:text-gray-700 hover:bg-gray-100 transition-colors">
        ...
        <form action="{{ route('kategori-menu.store') }}" method="POST" class="flex-1 overflow-y-auto flex flex-col justify-between">
            ...
            <div class="px-5 py-5 space-y-4">
                <div>
                    ...
                    <input type="text" name="nama_kategori" placeholder="Contoh: Makanan Utama, Paket Hemat, dll" required class="w-full px-3 py-2.5 text-sm border border-gray-200 rounded-2xl outline-none focus:ring-1 focus:ring-gray-400 focus:border-gray-400 transition-all">
                </div>

                <div>
                    <label class="block text-xs font-semibold text-gray-700 mb-1.5">Deskripsi</label>
                    <textarea name="deskripsi" placeholder="Tuliskan deskripsi kategori..." rows="3" class="w-full px-3 py-2.5 text-sm border border-gray-200 rounded-2xl outline-none focus:ring-1 focus:ring-gray-400 focus:border-gray-400 transition-all"></textarea>
                </div>
            </div>

            <div class="px-5 py-4 border-t border-gray-100 flex justify-end gap-2 bg-gray-50/80 shrink-0 mt-auto">
                <button type="button" onclick="closeModal('modalTambah', 'drawerTambahPanel')" class="text-sm font-medium text-gray-600 bg-white border border-gray-200 rounded-2xl px-4 py-2 hover:bg-gray-50 transition-colors">
                    Batal
                </button>
                <button type="submit" class="text-sm font-semibold text-white bg-gray-900 rounded-2xl px-5 py-2 hover:bg-gray-800 transition-colors">
                    Simpan Kategori
                </button>
            </div>
        </form>
...
<div id="modalEdit" class="fixed inset-x-0 bottom-0 top-16 z-50 hidden">
    ...
    <div class="absolute right-0 top-0 h-full w-full max-w-md bg-white shadow-2xl flex flex-col translate-x-full transition-transform duration-300" id="drawerEditPanel">
        <div class="flex items-center justify-between px-5 py-4 border-b border-gray-100 shrink-0">
            ...
            <button onclick="closeModal('modalEdit', 'drawerEditPanel')" class="p-1.5 rounded-2xl text-gray-400 hover:text-gray-700 hover:bg-gray-100 transition-colors">
                <svg xmlns="http://www.w3.org/2000/svg" class="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M6 18L18 6M6 6l12 12"/></svg>
            </button>
        </div>
        ...
        <form id="formEdit" method="POST" class="flex-1 overflow-y-auto flex flex-col justify-between">
            ...
            <div class="px-5 py-5 space-y-4">
                <div>
                    <label class="block text-xs font-semibold text-gray-700 mb-1.5">Nama Kategori <span class="text-red-500">*</span></label>
                    <input type="text" name="nama_kategori" id="editNama" required class="w-full px-3 py-2.5 text-sm border border-gray-200 rounded-2xl outline-none focus:ring-1 focus:ring-gray-400 focus:border-gray-400 transition-all">
                </div>

                <div>
                    <label class="block text-xs font-semibold text-gray-700 mb-1.5">Deskripsi</label>
                    <textarea name="deskripsi" id="editDeskripsi" placeholder="Tuliskan deskripsi kategori..." rows="3" class="w-full px-3 py-2.5 text-sm border border-gray-200 rounded-2xl bg-white outline-none focus:ring-1 focus:ring-gray-400 focus:border-gray-400 transition-all"></textarea>
                </div>
            </div>

            <div class="px-5 py-4 border-t border-gray-100 flex justify-end gap-2 bg-gray-50/80 shrink-0 mt-auto">
                <button type="button" onclick="closeModal('modalEdit', 'drawerEditPanel')" class="text-sm font-medium text-gray-600 bg-white border border-gray-200 rounded-2xl px-4 py-2 hover:bg-gray-50 transition-colors">
                    Batal
                </button>
                <button type="submit" class="text-sm font-semibold text-white bg-gray-900 rounded-2xl px-5 py-2 hover:bg-gray-800 transition-colors">
                    Simpan Perubahan
