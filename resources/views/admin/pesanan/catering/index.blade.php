...
<div class="flex-1 overflow-auto bg-gray-50/50 text-gray-800 font-sans">
    <div class="p-4 md:p-6 lg:p-8 max-w-[1400px] mx-auto space-y-6">
        ...
        {{-- Header --}}
        <x-ui.page-header title="Daftar Pesanan Catering" subtitle="Kelola seluruh transaksi pesanan catering, tanggal acara, rincian porsi & pembayaran DP">
        </x-ui.page-header>
        ...
        <x-ui.data-table :paginator="$pesanans">
            <x-slot:toolbar>
                <form action="{{ route('admin.pesanan.catering.index') }}" method="GET" class="w-full flex flex-col sm:flex-row gap-3">
            ...
            <table class="w-full text-left border-collapse min-w-[1100px]">
                <thead>
                    <tr class="bg-gray-50 text-gray-500 text-[11px] font-bold uppercase tracking-wider border-b border-gray-200 sticky top-0 z-10">
                        ...
                        <th class="px-4 py-3.5">Paket & Menu Terpilih</th>
                        ...
                        <tr class="hover:bg-blue-50/20 transition-colors align-top">
                            ...
                            <td class="px-4 py-3.5">
                                <div class="font-bold text-gray-900 flex items-center gap-1.5">
                                    <div class="w-6 h-6 rounded-full bg-blue-100 text-blue-700 font-bold text-[10px] flex items-center justify-center flex-shrink-0">
                            ...
                            <!-- 3. Paket & Menu Terpilih -->
                            <td class="px-4 py-3.5 max-w-[260px]">
                                <div class="font-bold text-gray-900 text-xs">
                                    {{ $p->paket->nama_paket ?? 'Paket Custom' }}
                                </div>
                                <div class="inline-flex items-center px-2 py-0.5 rounded text-[10px] font-extrabold bg-blue-50 text-blue-700 border border-blue-100 mt-1">
                            ...
                            <td class="px-4 py-3.5">
                                ...
                                @if($p->dp_amount)
                                    <div class="text-[10px] text-gray-500 font-medium mt-0.5">
                                        DP: <span class="font-bold text-blue-600">Rp {{ number_format($p->dp_amount, 0, ',', '.') }}</span>
                                    </div>
                                @endif
                            </td>
                            ...
                            <td class="px-4 py-3.5 text-right">
                                <x-ui.button href="{{ route('admin.pesanan.catering.show', $p->id) }}" size="sm">
                    ...
                    @empty
                        <tr>
                            <td colspan="6" class="py-10 text-center">
                                <x-ui.empty-state icon="fa-receipt" title="Tidak ada data pesanan catering" />
                            </td>
                        </tr>
                    @endforelse
                </tbody>
