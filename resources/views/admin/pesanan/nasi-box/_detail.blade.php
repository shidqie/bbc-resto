@php
        ...
        $alamatKonsumen = $konsumen->alamat ?? $pesanan->jadwal_pesanan->alamat_pengiriman ?? $pesanan->catatan ?? '-';
        ...
        $jenisPesanan = $pesanan->jenis_pesanan->nama_jenis ?? 'Nasi Box';
        $namaPaket = $detailPesanan->menu->nama_menu ?? 'Nasi Box';
    ...
    <div class="py-4" id="detail-pesanan-nasibox-{{ $pesanan->id }}"
         x-data="{
            showPengingat: false,
            ...
            updatePesan() {
                ...
                if (this.jenisPengingat === 'pesanan') {
                    this.pesanWa = `Halo ${this.nama}, kami dari RM BBC ingin mengonfirmasi & mengingatkan pesanan ${this.jenis} ${this.paket} sebanyak ${this.porsi} box (ID Pesanan: #${this.kode}) untuk tanggal ${this.tanggal}${waktuStr}.\n\nStatus Pembayaran: ${this.statusBayar}\n\nUntuk melihat rincian dan melacak status pesanan Anda, silakan kunjungi:\n${linkLacak}\n\nMohon pastikan kembali bahwa rincian pesanan dan jadwal acara sudah sesuai. Terima kasih.`;
                } else if (this.jenisPengingat === 'pembayaran_dp') {
                    if (this.isSkemaLunas) {
                        this.pesanWa = `Halo ${this.nama}, kami dari RM BBC ingin mengingatkan pembayaran untuk pesanan ${this.jenis} ${this.paket} (${this.porsi} box) dengan ID Pesanan: #${this.kode}.\n\nNominal Tagihan: Rp ${this.nominalDp}\n\nSilakan selesaikan pembayaran dan unggah bukti transfer melalui tautan berikut:\n${linkBayar}\n\nLacak status pesanan:\n${linkLacak}\n\nTerima kasih.`;
                    } else {
                        this.pesanWa = `Halo ${this.nama}, kami dari RM BBC ingin mengingatkan pembayaran Uang Muka (DP 25%) untuk pesanan ${this.jenis} ${this.paket} (${this.porsi} box) dengan ID Pesanan: #${this.kode}.\n\nNominal DP: Rp ${this.nominalDp}\n\nSilakan selesaikan pembayaran dan unggah bukti transfer melalui tautan berikut:\n${linkBayar}\n\nLacak status pesanan:\n${linkLacak}\n\nTerima kasih.`;
                    }
                } else if (this.jenisPengingat === 'pelunasan') {
                    this.pesanWa = `Halo ${this.nama}, kami dari RM BBC ingin mengingatkan bahwa pesanan ${this.jenis} ${this.paket} (${this.porsi} box) untuk tanggal ${this.tanggal} masih memiliki sisa pelunasan sebesar Rp ${this.sisaTagihan}.\n\nBatas Pelunasan: Maksimal H-3 sebelum acara (${this.batasPelunasan})\n\nSilakan selesaikan pelunasan melalui tautan berikut:\n${linkBayar}\n\nLacak status pesanan:\n${linkLacak}\n\nTerima kasih.`;
                } else if (this.jenisPengingat === 'hari_acara') {
                    this.pesanWa = `Halo ${this.nama}, kami dari RM BBC menginformasikan bahwa pesanan ${this.jenis} ${this.paket} (${this.porsi} box) dengan kode #${this.kode} dijadwalkan untuk acara pada tanggal ${this.tanggal}${waktuStr}.\n\nLokasi / Pengiriman: ${this.alamat}\n\nLacak status pesanan:\n${linkLacak}\n\nTim kami sedang mempersiapkan pesanan Anda dengan sebaik-baiknya. Terima kasih.`;
                } else if (this.jenisPengingat === 'pengiriman') {
                    this.pesanWa = `Halo ${this.nama}, kami dari RM BBC menginformasikan bahwa pesanan ${this.jenis} ${this.paket} (${this.porsi} box) dengan kode #${this.kode} sedang dipersiapkan untuk ${this.metodeKirim} ke alamat:\n${this.alamat}\n\nLacak status & pengiriman secara langsung:\n${linkLacak}\n\nTerima kasih atas kepercayaan Anda.`;
                }
            },
            bukaModal() {
        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8 space-y-6">
            ...
            {{-- Top Detail Action Bar with Close Button --}}
            <div class="flex flex-wrap items-center justify-between gap-3 pb-3 border-b border-gray-200 bg-white p-4 rounded-xl shadow-xs">
                <div class="flex flex-wrap items-center gap-3">
                    ...
                    @endif
                    <span class="text-xs text-gray-500 font-medium hidden sm:inline">&bull; {{ $detailPesanan->menu->nama_menu ?? 'Nasi Box' }} ({{ $detailPesanan->jumlah ?? 0 }} Box)</span>
                </div>
                <div class="flex items-center gap-2">
                    @if(!$isDapur)
                    <a href="{{ route('admin.pesanan.nasibox.pdf', $pesanan->id) }}" 
                       target="_blank" 
                    ...
                    @endif
                    <button type="button" @click="$dispatch('close-nasibox-drawer'); open = false;" class="inline-flex items-center gap-1.5 px-4 py-2 bg-gray-100 hover:bg-red-50 hover:text-red-700 text-gray-700 font-bold text-xs rounded-xl transition-all border border-gray-200 hover:border-red-200 cursor-pointer shadow-xs">
                ...
                <div class="p-6 text-gray-900 space-y-6">
                        ...
                        <div class="space-y-2.5 text-sm">
                            ...
                            <div class="grid grid-cols-3 items-center"><span class="text-gray-500">Nomor WhatsApp</span>
                                <div class="col-span-2 flex items-center gap-2.5">
                                    @if($waLink)
                                        <span class="font-mono text-gray-900 font-semibold">{{ $kontakKonsumen ? \App\Support\WhatsAppNumber::formatForDisplay($kontakKonsumen) : '-' }}</span>
                                        <a href="{{ $waLink }}" target="_blank" class="inline-flex items-center gap-1 px-2.5 py-1 bg-emerald-50 hover:bg-emerald-100 text-emerald-700 border border-emerald-200 rounded-lg text-xs font-bold transition-colors">
                    ...
                    <div>
                        ...
                        <div class="space-y-2.5 text-sm">
                            ...
                            <div class="grid grid-cols-3"><span class="text-gray-500">Jenis Pesanan</span> <span class="col-span-2 font-semibold text-gray-900">{{ $pesanan->jenis_pesanan->nama_jenis ?? 'Nasi Box' }}</span></div>
                            <div class="grid grid-cols-3"><span class="text-gray-500">Varian</span> <span class="col-span-2 font-semibold text-emerald-800">{{ $detailPesanan->menu->nama_menu ?? '-' }}</span></div>
                            <div class="grid grid-cols-3"><span class="text-gray-500">Jumlah Box</span> <span class="col-span-2 font-bold text-gray-900">{{ $detailPesanan->jumlah ?? 0 }} Box</span></div>
                            <div class="grid grid-cols-3"><span class="text-gray-500">Tanggal Pesan</span> <span class="col-span-2 text-gray-700">{{ $pesanan->dibuat_pada ? \Carbon\Carbon::parse($pesanan->dibuat_pada)->translatedFormat('d M Y') : '-' }}</span></div>
                    ...
                    <div>
                        ...
                        <div class="space-y-2.5 text-sm">
                            ...
                            <div class="grid grid-cols-3">
                                <span class="text-gray-500">No. DO</span>
                                        ...
                                        <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-2.5">
                                            ...
                                            <div class="flex items-center gap-1.5 shrink-0 pt-2 sm:pt-0 border-t sm:border-t-0 border-gray-100">
                                                ...
                                                @if($isAdminOrPemilik && $isPending)
                                                    <form id="form-verif-nb-{{ $pemb->id }}" action="{{ route('admin.bukti.nasibox.verifikasi-pembayaran', $pemb->id) }}" method="POST" class="hidden">
                                                        @csrf @method('PATCH')
                                                    </form>
                                                    <x-ui.action-button type="button"
                                                        onclick="window.confirmDialog({ title: 'Verifikasi Pembayaran', name: '{{ $pemb->kode_pembayaran }}', message: 'Setujui bukti pembayaran ini? Status pesanan akan diperbarui secara otomatis.', formId: 'form-verif-nb-{{ $pemb->id }}', confirmText: 'Setujui', cancelText: 'Batal' })"
                                                        title="Setujui"
                                                        class="text-green-600 hover:text-green-800">
                                                        <x-heroicon-o-check-circle class="w-4 h-4" />
                                                    </x-ui.action-button>

                                                    <form id="form-tolak-nb-{{ $pemb->id }}" action="{{ route('admin.bukti.nasibox.tolak-pembayaran', $pemb->id) }}" method="POST" class="hidden">
                                                        @csrf @method('PATCH')
                                                        <input type="hidden" name="catatan_admin" value="">
                                                    </form>
                                                    <x-ui.action-button type="button"
                                                        onclick="window.confirmDialog({ title: 'Tolak Bukti Pembayaran', name: '{{ $pemb->kode_pembayaran }}', message: 'Tolak bukti pembayaran ini?', formId: 'form-tolak-nb-{{ $pemb->id }}', confirmText: 'Tolak', cancelText: 'Batal', type: 'danger' })"
                                                        title="Tolak"
                                ...
                                <table class="w-full text-left border-collapse text-sm">
                                     <thead>
                                        <tr class="bg-gray-50/80 border-b border-gray-200 text-xs font-bold text-gray-500 uppercase tracking-wider">
                                            ...
                                            <th class="py-3 px-4 text-center w-36">Stok Harian</th>
                                    ...
                                    <tbody class="divide-y divide-gray-100">
                                        ...
                                        @php
                                            ...
                                            $stokVal = (float) ($bb['stok_harian'] ?? $bb['stok_katering'] ?? 0);
            ...
            @if($isAdminOrPemilik)
                @if($pesanan->status_pesanan_id == 6 && $pesanan->catatan && str_contains($pesanan->catatan, '[BATAL:'))
                    <div class="mt-6 bg-red-50 p-4 rounded-xl border border-red-100">
                        <h4 class="text-red-800 font-bold mb-1 flex items-center gap-1.5"><x-heroicon-o-no-symbol class="w-5 h-5" />Alasan Pembatalan:</h4>
                        <p class="text-red-700 text-sm">{{ Str::after($pesanan->catatan, '[BATAL:') }}</p>
                    </div>
                @endif

                {{-- Tombol Aksi Status Multiaktor --}}
                @php
                ...
                @if(!in_array($stPesananId, [5, 6]))
                    <div class="bg-white overflow-hidden shadow-sm sm:rounded-2xl border border-gray-200 p-6" x-data="{ showBatalModal: false }">
                        <div class="flex flex-wrap gap-3 justify-center items-center">
                            ...
                            @if($isPemilikActor)
                                ...
                                @if($stPesananId === 1)
                                    @if(in_array($stBayarId, [3, 4, 5]))
                                        <form id="form-konfirmasi-nasibox" action="{{ route('admin.pesanan.nasibox.update-status', $pesanan->id) }}" method="POST">
                                            ...
                                            <button type="button" onclick="window.confirmDialog({ title: 'Konfirmasi Pesanan', name: '{{ $pesanan->id_pesanan }}', message: 'Konfirmasi pesanan Nasi Box ini?', formId: 'form-konfirmasi-nasibox', confirmText: 'Konfirmasi', cancelText: 'Batal', type: 'warning' })" class="bg-emerald-600 hover:bg-emerald-700 text-white font-bold py-3 px-8 rounded-xl shadow-xs cursor-pointer transition-all flex items-center">
                                ...
                                @if($stPesananId === 2)
                                    <form id="form-jadwalkan-nasibox" action="{{ route('admin.pesanan.nasibox.update-status', $pesanan->id) }}" method="POST">
                                        @csrf
                                        @method('PATCH')
                                        <input type="hidden" name="status" value="7">
                                        <button type="button" onclick="window.confirmDialog({ title: 'Tandai Terjadwal', name: '{{ $pesanan->id_pesanan }}', message: 'Tandai pesanan Nasi Box ini sebagai Terjadwal?', formId: 'form-jadwalkan-nasibox', confirmText: 'Tandai Terjadwal', cancelText: 'Batal', type: 'warning' })" class="bg-blue-600 hover:bg-blue-700 text-white font-bold py-3 px-8 rounded-xl shadow-xs cursor-pointer transition-all flex items-center">
                                ...
                                @if($stPesananId === 4 && !$hasPengiriman)
                                    <form id="form-selesai-nasibox" action="{{ route('admin.pesanan.nasibox.update-status', $pesanan->id) }}" method="POST">
                                        ...
                                        <button type="button" onclick="window.confirmDialog({ title: 'Pesanan Selesai', name: '{{ $pesanan->id_pesanan }}', message: 'Tandai pesanan sebagai selesai (sudah diambil oleh konsumen)?', formId: 'form-selesai-nasibox', confirmText: 'Selesai', cancelText: 'Batal', type: 'warning' })" class="bg-emerald-600 hover:bg-emerald-700 text-white font-bold py-3 px-8 rounded-xl shadow-xs cursor-pointer transition-all flex items-center">
                            ...
                            @if($isDapurActor)
                                ...
                                @if(in_array($stPesananId, [2, 7]))
                                    @if(in_array($stBayarId, [3, 4, 5]))
                                        <form id="form-produksi-nasibox" action="{{ route('admin.pesanan.nasibox.update-status', $pesanan->id) }}" method="POST">
                                            ...
                                            <button type="button" onclick="window.confirmDialog({ title: 'Mulai Sedang Diproses', name: '{{ $pesanan->id_pesanan }}', message: 'Mulai proses dapur? Stok bahan baku akan dipotong otomatis.', formId: 'form-produksi-nasibox', confirmText: 'Mulai Masak', cancelText: 'Batal', type: 'warning' })" class="bg-amber-600 hover:bg-amber-700 text-white font-bold py-3 px-8 rounded-xl shadow-xs cursor-pointer transition-all flex items-center">
                                ...
                                @if($stPesananId === 3)
                                    <form id="form-produksi-selesai-nasibox" action="{{ route('admin.pesanan.nasibox.update-status', $pesanan->id) }}" method="POST">
                                        ...
                                        <button type="button" onclick="window.confirmDialog({ title: 'Tandai Pesanan Siap', name: '{{ $pesanan->id_pesanan }}', message: 'Tandai seluruh hidangan Nasi Box telah selesai disiapkan oleh Dapur?', formId: 'form-produksi-selesai-nasibox', confirmText: 'Pesanan Siap', cancelText: 'Batal', type: 'warning' })" class="bg-purple-600 hover:bg-purple-700 text-white font-bold py-3 px-8 rounded-xl shadow-xs cursor-pointer transition-all flex items-center">
                            ...
                            {{-- Tombol Tutup Drawer --}}
                            <button type="button" @click="$dispatch('close-nasibox-drawer'); open = false;" class="bg-gray-100 hover:bg-gray-200 text-gray-700 font-bold py-3 px-8 rounded-xl shadow-xs border border-gray-200 flex items-center cursor-pointer transition-all">
