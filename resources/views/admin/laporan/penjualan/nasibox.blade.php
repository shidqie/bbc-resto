...
<div class="flex-1 overflow-auto bg-gray-50/50 text-gray-800 font-sans">
    <div class="w-full p-6 ] space-y-6">
        ...
        <x-ui.page-header title="Laporan Pendapatan Nasi Box" subtitle="Ringkasan transaksi nasi box yang telah selesai" :breadcrumbs="['Laporan', 'Pendapatan Nasi Box']">
            <x-slot:actions>
                <a href="{{ route('laporan.nasibox.cetak', ['start_date' => $startDate, 'end_date' => $endDate]) }}" target="_blank" class="inline-flex items-center gap-1.5 text-sm font-semibold text-gray-700 bg-white border border-gray-200 rounded-lg px-3 py-2 hover:bg-gray-50 transition-colors shadow-sm">
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-2 gap-4">
            <div class="bg-white p-4 rounded-xl border border-gray-200 shadow-sm flex flex-col justify-center">
                <span class="text-xs font-semibold text-gray-500 uppercase tracking-wider mb-1">Total Pendapatan (Nasi Box)</span>
        ...
        <x-ui.data-table>
            <table class="w-full text-left border-collapse min-w-[800px]">
                <x-ui.table.header>
                    ...
                    <th class="px-4 py-3.5 text-left">Paket Nasi Box</th>
                ...
                <tbody class="divide-y divide-gray-100 text-sm">
                    ...
                    @empty
                        <x-empty-state icon="document-text" title="Tidak ada data nasi box" :colspan="5" />
                    @endforelse
                </tbody>
                @if($pesanans->count() > 0)
                    <tfoot>
