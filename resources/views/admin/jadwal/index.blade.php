...
<div class="flex-1 bg-gray-50 text-gray-800 font-sans">
    <div class="w-full px-4 md:px-6 py-6 space-y-5">
        ...
        <div class="flex items-center justify-between gap-3">
            <div>
                <h1 class="text-lg font-bold text-gray-900">Jadwal Pengantaran</h1>
                <p class="text-xs text-gray-500 mt-0.5">Kelola jadwal pengiriman pesanan Catering &amp; Nasi Box</p>
            </div>
        ...
        <div class="grid grid-cols-2 md:grid-cols-4 gap-3">
            ...
            <div class="bg-white rounded-xl border border-gray-200 px-4 py-3">
                <p class="text-xs font-medium text-gray-500">Menunggu / Diproses</p>
                <p class="text-xl font-bold text-amber-600 mt-1">{{ $summary['diproses'] + $summary['menunggu_konfirmasi'] }}</p>
            </div>
            <div class="bg-white rounded-xl border border-gray-200 px-4 py-3">
                <p class="text-xs font-medium text-gray-500">Sedang Diantar</p>
                <p class="text-xl font-bold text-purple-600 mt-1">{{ $summary['dikirim'] }}</p>
            </div>
            <div class="bg-white rounded-xl border border-gray-200 px-4 py-3">
                <p class="text-xs font-medium text-gray-500">Selesai</p>
                <p class="text-xl font-bold text-emerald-600 mt-1">{{ $summary['selesai'] + $summary['lunas'] }}</p>
            </div>
        </div>
            ...
            <table class="w-full text-sm min-w-[900px]">
                ...
                <tbody class="divide-y divide-gray-50">
                    ...
                    <tr class="hover:bg-gray-50/60 transition-colors group align-top">
                        ...
                        <td class="px-4 py-3">
                            <div class="flex items-center gap-2 mb-1">
                                <span class="font-bold text-gray-900 text-sm">
                                    {{ $order->waktu_acara ? \Carbon\Carbon::parse($order->waktu_acara)->format('H:i') : 'Belum Diset' }}
                                </span>
                            </div>
                            <p class="font-semibold text-gray-600 font-mono text-[10px]">{{ $order->kode_pesanan }}</p>
                            @if($order->jenis === 'Catering')
                        ...
                        {{-- Pelanggan --}}
                        <td class="px-4 py-3">
                            <p class="font-semibold text-gray-900 text-xs">{{ $order->nama_pemesan }}</p>
                        ...
                        <td class="px-4 py-3 max-w-[200px]">
                            <p class="text-xs text-gray-700 leading-relaxed">
                                {{ $order->jenis == 'Catering' ? $order->lokasi_acara : $order->alamat }}
                            </p>
                        </td>

                        {{-- Rincian Paket --}}
                        <td class="px-4 py-3">
                            <p class="text-xs font-semibold text-gray-800">{{ $order->paket->nama_paket ?? 'Paket Kustom' }}</p>
                            <p class="text-[10px] text-gray-500 mt-0.5">{{ $order->jenis == 'Catering' ? $order->jumlah_porsi . ' Porsi' : $order->jumlah_box . ' Box' }}</p>
                        </td>

                        {{-- Status Pengantaran --}}
                        <td class="px-4 py-3">
                            <form action="{{ route('admin.jadwal.update-status', ['jenis' => $order->jenis, 'id' => $order->id]) }}" method="POST">
                                @csrf @method('PATCH')
                                <select name="status" onchange="this.form.submit()"
                                        class="text-[11px] font-semibold rounded-lg border px-2 py-1 outline-none cursor-pointer transition-colors w-full
                                               {{ in_array($order->status, ['diproses', 'menunggu_konfirmasi']) ? 'bg-amber-50 text-amber-900 border-amber-200' : '' }}
                                               {{ $order->status == 'dikirim' ? 'bg-purple-50 text-purple-900 border-purple-200' : '' }}
                                               {{ in_array($order->status, ['selesai', 'lunas']) ? 'bg-emerald-50 text-emerald-900 border-emerald-200' : '' }}
                                               {{ $order->status == 'dibatalkan' ? 'bg-red-50 text-red-800 border-red-200' : '' }}">
                                    <option value="menunggu_konfirmasi" {{ in_array($order->status, ['menunggu_dp', 'menunggu_konfirmasi', 'terkonfirmasi']) ? 'selected' : '' }}>• Menunggu</option>
                                    <option value="diproses" {{ $order->status == 'diproses' ? 'selected' : '' }}>• Diproses (Dapur)</option>
                                    <option value="dikirim" {{ $order->status == 'dikirim' ? 'selected' : '' }}>• Sedang Diantar</option>
                                    <option value="selesai" {{ in_array($order->status, ['selesai', 'lunas']) ? 'selected' : '' }}>• Selesai</option>
                                </select>
                            </form>
                        </td>
                        ...
                        <td class="px-4 py-3 text-right">
                            <div class="flex items-center justify-end gap-1 opacity-70 group-hover:opacity-100 transition-opacity">
                                <a href="{{ $order->jenis === 'Catering' ? url('/admin/pesanan/catering/' . $order->id) : url('/admin/pesanan/nasi-box/' . $order->id) }}" 
