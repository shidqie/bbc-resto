    ...
    <div class="min-h-screen bg-surface text-body">
        <header class="py-12 border-b border-gray-100">
            <div class="max-w-6xl mx-auto px-6 md:px-12">
                <h1 class="text-[40px] font-medium leading-tight tracking-tight mb-2 text-primary">Bayar Tagihan</h1>
        ...
        <main class="max-w-6xl mx-auto px-6 md:px-12 py-12 lg:py-16">
            ...
            <div class="max-w-2xl mx-auto">
                <div class="border-2 border-primary/15 rounded-xl p-8 bg-emerald-50/40">
                    <div class="w-12 h-12 rounded-xl bg-primary text-accent flex items-center justify-center mb-5">
                    ...
                    <p class="text-sm text-gray-500 mb-6">Nomor invoice tertera pada bukti pesanan, contoh: <span class="font-mono text-primary font-medium">CTR-20260802-001</span></p>
                    ...
                    <form action="{{ route('pesanan.bayar.cari') }}" method="GET" class="space-y-4">
                        <div>
                            ...
                            <input type="text" id="kode_pesanan" name="kode_pesanan" value="{{ old('kode_pesanan') }}" required placeholder="Contoh: CTR-20260802-001"
                                   class="w-full px-4 py-3.5 rounded-xl border border-gray-200 bg-white text-gray-900 text-sm focus:outline-none focus:ring-2 focus:ring-primary/30 focus:border-primary placeholder:text-gray-300">
                            @error('kode_pesanan') <p class="text-xs text-red-600 mt-1">{{ $message }}</p> @enderror
                        </div>
                        <button type="submit" class="w-full flex items-center justify-center gap-2 px-6 py-3.5 bg-primary hover:bg-primary-container text-white font-bold tracking-widest text-sm uppercase rounded-xl transition-all border border-primary active:scale-[0.99]">
