...
<div class="flex-1 bg-gray-50 text-gray-800">
    <div class="w-full p-6 space-y-5">
        ...
        {{-- Tabs --}}
        <div class="flex items-center gap-1 bg-gray-100 p-1 rounded-xl border border-gray-200/60 mb-5 w-fit">
            <a href="{{ route('stok-operasional.index') }}"
               class="inline-flex items-center gap-2 px-4 h-9 rounded-lg text-sm transition-all {{ request()->routeIs('stok-operasional.*') ? 'bg-white text-gray-900 shadow-sm font-bold' : 'text-gray-500 font-semibold hover:text-gray-900 hover:bg-gray-200/50' }}">
                <x-heroicon-o-building-storefront class="shrink-0 text-center w-3 h-3" />
                <span>Dine In & Nasi Box</span>
            </a>
            <a href="{{ route('stok-catering.index') }}"
               class="inline-flex items-center gap-2 px-4 h-9 rounded-lg text-sm transition-all {{ request()->routeIs('stok-catering.*') ? 'bg-white text-gray-900 shadow-sm font-bold' : 'text-gray-500 font-semibold hover:text-gray-900 hover:bg-gray-200/50' }}">
                <x-heroicon-o-truck class="shrink-0 text-center w-3 h-3" />
                <span>Catering</span>
            </a>
        </div>
                ...
                <tbody class="divide-y divide-gray-50">
                    ...
                    <tr class="hover:bg-gray-50/60 transition-colors {{ $isHabis ? 'bg-red-50/30' : ($isMenipis ? 'bg-amber-50/30' : '') }}">
                        ...
                        <td class="px-4 py-3">
                            ...
                            @else
                                <span class="inline-flex items-center gap-1 text-xs font-semibold px-2 py-0.5 rounded-md bg-emerald-50 text-emerald-700">
                        ...
                        <td class="px-4 py-3 text-center">
                            <div class="flex items-center justify-center gap-1.5">
                                <a href="{{ route('bahan-baku.show', $bahan->id) }}" title="Detail" class="w-7 h-7 rounded-lg flex items-center justify-center bg-blue-50 text-blue-600 hover:bg-blue-100 transition-colors">
                                    <x-heroicon-o-eye class="w-3 h-3" />
                                </a>
                                <button type="button" onclick="alert('Fitur Penyesuaian Stok belum tersedia')" title="Penyesuaian" class="w-7 h-7 rounded-lg flex items-center justify-center bg-amber-50 text-amber-600 hover:bg-amber-100 transition-colors">
