...
<div class="flex-1 overflow-auto bg-gray-50/50 text-gray-800 font-sans">
    <div class="w-full p-6 ] space-y-6">
        ...
        {{-- Header --}}
        <x-ui.page-header 
            title="Edit Bahan Baku: {{ $bahanBaku->nama_bahan_baku }}" 
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    ...
                    <div class="space-y-6">
                        <div>
                            ...
                            <input type="text" name="kode_bahan" value="{{ $bahanBaku->kode_bahan_baku }}" readonly class="bg-gray-100 border border-gray-200 text-gray-500 text-sm rounded-xl block w-full p-3 cursor-not-allowed outline-none">
                        </div>
                        
                        <div>
                            <label class="block text-sm font-medium text-gray-700 mb-1">Nama Bahan Baku <span class="text-red-500">*</span></label>
                            <input type="text" name="nama_bahan" value="{{ old('nama_bahan', $bahanBaku->nama_bahan_baku) }}" required class="bg-gray-50 border border-gray-200 text-gray-900 text-sm rounded-xl focus:ring-2 focus:ring-[#0F2E23]/10 focus:border-[#0F2E23] outline-none block w-full p-3 transition-colors placeholder-gray-400 font-medium" placeholder="Contoh: Beras Premium">
                        </div>
                        ...
                        <div>
                            ...
                            <select name="status" required class="bg-gray-50 border border-gray-200 text-gray-900 text-sm rounded-xl focus:ring-2 focus:ring-blue-100 focus:border-[#3B82F6] outline-none block w-full p-3 transition-colors">
                                <option value="1" {{ old('status', $bahanBaku->status_aktif) == '1' ? 'selected' : '' }}>Aktif</option>
                                <option value="0" {{ old('status', $bahanBaku->status_aktif) == '0' ? 'selected' : '' }}>Nonaktif</option>
                            </select>
                        </div>
                    </div>
                    ...
                    <div class="space-y-6">
                        <div class="grid grid-cols-2 gap-4">
                            ...
                            <div>
                                ...
                                <input type="number" step="0.01" min="0" name="stok_minimum" value="{{ old('stok_minimum', $bahanBaku->stok_minimal) }}" required class="bg-gray-50 border border-gray-200 text-gray-900 text-sm rounded-xl focus:ring-2 focus:ring-blue-100 focus:border-[#3B82F6] outline-none block w-full p-3 transition-colors">
                            </div>
                        </div>
                        ...
                        <div>
                            ...
                            <textarea name="keterangan" rows="4" class="bg-gray-50 border border-gray-200 text-gray-900 text-sm rounded-xl focus:ring-2 focus:ring-blue-100 focus:border-[#3B82F6] outline-none block w-full p-3 transition-colors placeholder-gray-400" placeholder="Deskripsi tambahan bahan baku...">{{ old('keterangan', $bahanBaku->deskripsi) }}</textarea>
                        </div>
