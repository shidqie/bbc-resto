...
<div class="flex-1 bg-gray-50 text-gray-800">
    <div class="w-full p-6 space-y-5">
        ...
        <div class="flex items-center justify-between gap-3">
            ...
            <a href="{{ route('bahan-baku.create') }}" class="inline-flex items-center gap-1.5 text-sm font-semibold text-white bg-gray-900 rounded-2xl px-3 py-2 hover:bg-gray-800 transition-colors">
        ...
        <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-5">
            <div class="bg-white rounded-2xl p-5 border border-neutral-200 flex flex-col gap-3">
                <div class="flex items-center justify-between text-neutral-500">
                    <p class="text-xs font-medium">Total Bahan Baku</p>
                    <x-heroicon-o-cube class="w-4 h-4 text-neutral-400" />
                </div>
                <div>
                    <p class="text-2xl font-semibold text-neutral-900">{{ $totalBahan }}</p>
                </div>
            </div>
            <div class="bg-white rounded-2xl p-5 border border-neutral-200 flex flex-col gap-3">
                <div class="flex items-center justify-between text-neutral-500">
                    <p class="text-xs font-medium">Stok Aman</p>
                    <x-heroicon-o-check-circle class="w-4 h-4 text-neutral-400" />
                </div>
                <div>
                    <p class="text-2xl font-semibold text-neutral-900">{{ $stokAman }}</p>
                </div>
            </div>
            <div class="bg-white rounded-2xl p-5 border border-neutral-200 flex flex-col gap-3">
                <div class="flex items-center justify-between text-neutral-500">
                    <p class="text-xs font-medium">Stok Menipis</p>
                    <x-heroicon-o-exclamation-triangle class="w-4 h-4 text-neutral-400" />
                </div>
                <div>
            ...
            <div class="bg-white rounded-2xl p-5 border border-neutral-200 flex flex-col gap-3">
                <div class="flex items-center justify-between text-neutral-500">
                    <p class="text-xs font-medium">Stok Habis</p>
                    <x-heroicon-o-x-circle class="w-4 h-4 text-neutral-400" />
                </div>
                <div>
                    <p class="text-2xl font-semibold text-neutral-900">{{ $stokHabis }}</p>
                </div>
            </div>
        </div>
        ...
        <div class="flex flex-col sm:flex-row gap-2 items-start sm:items-center justify-between mb-3 shrink-0">
            <form action="{{ route('bahan-baku.index') }}" method="GET" class="flex items-center gap-2 w-full sm:w-auto flex-wrap">
                <div class="relative flex-1 sm:flex-none sm:w-56">
                    ...
                    <input type="text" name="search" value="{{ request('search') }}" placeholder="Cari kode atau nama bahan…" class="w-full pl-8 pr-3 py-2 text-xs border border-gray-200 rounded-2xl outline-none focus:ring-1 focus:ring-gray-400 focus:border-gray-400 transition-all bg-white">
                </div>
                ...
                <select name="status_stok" class="text-xs border border-gray-200 rounded-2xl bg-white px-3 py-2 outline-none focus:ring-1 focus:ring-gray-400 transition-all" onchange="this.form.submit()">
                ...
                @if(request()->hasAny(['search', 'kategori', 'status_stok']))
                    <a href="{{ route('bahan-baku.index') }}" class="text-xs font-medium text-red-500 hover:text-red-700 px-2 py-2 rounded-2xl hover:bg-red-50 transition-colors shrink-0">Reset</a>
                @endif
            </form>
        </div>
