...
<head>
    ...
    <title>Struk Dapur #{{ $pesanan->nomor_pesanan }}</title>
    <style>
    ...
    <div class="border-top border-bottom mb-2 mt-2 font-bold">
        <table style="margin:0; font-size: 16px;">
            <tr><td style="width: 40%;">Meja</td><td>: {{ $pesanan->meja->nomor_meja ?? '-' }}</td></tr>
            <tr><td>Jenis</td><td>: Dine In</td></tr>
            <tr><td>No</td><td>: {{ substr($pesanan->nomor_pesanan, -4) }}</td></tr>
            <tr><td style="font-size: 12px; font-weight: normal;">Waktu</td><td style="font-size: 12px; font-weight: normal;">: {{ $pesanan->dibuat_pada->format('H:i') }}</td></tr>
        </table>
    </div>
    ...
    <table>
        ...
        <tbody>
            ...
            <tr>
                ...
                <td>
                    <span class="font-bold" style="font-size: 16px;">{{ $detail->menu->nama_menu ?? $detail->menu->nama ?? 'Menu' }}</span>
                    @if($detail->catatan)
