...
<div class="h-[calc(100vh-65px)] flex items-center justify-center bg-[#f5f5f0] text-[#111827] font-sans">
    ...
    <div class="bg-white rounded-[32px] p-8 max-w-sm w-full shadow-lg border border-gray-100 flex flex-col items-center text-center space-y-6 relative overflow-hidden">
        ...
        <div class="relative z-10 text-center">
            ...
            <p class="text-sm font-medium text-gray-500 mt-2">
                Pesanan <span class="font-bold text-[#0F2E23]">#{{ $pesanan->kode_pesanan ?? 'DIN-'.$pesanan->id }}</span> telah berhasil dibayar.
            </p>
        </div>
        ...
        <div class="w-full bg-gray-50 border border-gray-100 rounded-2xl p-4 space-y-2 relative z-10">
            ...
            <div class="flex justify-between text-xs font-bold text-gray-500">
                ...
                <span class="uppercase">{{ optional($pesanan->pembayaran)->metode_pembayaran ?? 'CASH' }}</span>
            </div>
            @if(optional($pesanan->pembayaran)->jumlah_bayar > 0)
            <div class="flex justify-between text-xs font-bold text-gray-500">
                <span>Jumlah Bayar</span>
                <span>Rp {{ number_format(optional($pesanan->pembayaran)->jumlah_bayar, 0, ',', '.') }}</span>
            </div>
            @endif
            @if(optional($pesanan->pembayaran)->metode_pembayaran === 'cash' && optional($pesanan->pembayaran)->jumlah_bayar > $pesanan->total_harga)
                <div class="border-t border-gray-200/60 pt-2 mt-2 flex justify-between text-sm">
                    <span class="font-bold text-gray-600">Kembalian</span>
                    <span class="font-black text-emerald-600">Rp {{ number_format(optional($pesanan->pembayaran)->jumlah_bayar - $pesanan->total_harga, 0, ',', '.') }}</span>
                </div>
            @endif
        </div>
                        ...
                        <div class="bg-white rounded-[24px] p-6 max-w-lg w-full shadow-2xl border border-gray-100 relative" @click.outside="showSavePrintModal = false">
                            ...
                            <div class="space-y-3">
                                ...
                                <button type="button" onclick="printSilentIframe('/pos/dinein/pesanan/{{ $pesanan->id }}/print-nota')" class="w-full group bg-white border border-gray-100 hover:border-emerald-200 hover:bg-emerald-50/30 rounded-2xl p-4 flex items-center justify-between transition-all cursor-pointer">
                                    ...
                                    <i class="fa-solid fa-chevron-right text-gray-300 group-hover:text-emerald-500 text-xs"></i>
                                </button>
