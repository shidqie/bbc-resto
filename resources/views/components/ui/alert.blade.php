...
@if(session('success'))
    <div class="bg-emerald-50 border border-emerald-200 text-emerald-800 px-4 py-3 rounded-lg flex items-center gap-3">
        ...
        <p class="text-sm font-medium">{{ is_array(session('success')) ? implode(', ', session('success')) : session('success') }}</p>
    </div>
@endif
...
@if(session('error'))
    <div class="bg-red-50 border border-red-200 text-red-800 px-4 py-3 rounded-lg flex items-center gap-3">
        ...
        <p class="text-sm font-medium">{{ is_array(session('error')) ? implode(', ', session('error')) : session('error') }}</p>
    </div>
@endif
...
@if(session('warning') || session('warning_bom'))
    @php
        $warningMsg = session('warning') ?? session('warning_bom');
    @endphp
    <div class="bg-amber-50 border border-amber-200 text-amber-800 px-4 py-3 rounded-lg flex items-start gap-3">
        ...
        <p class="text-sm font-medium">{{ is_array($warningMsg) ? implode(', ', $warningMsg) : $warningMsg }}</p>
    </div>
@endif
