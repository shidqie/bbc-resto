...
<div onclick="openMenuModal({ id: {{ $menu->id ?? $menu->id_menu }}, nama: '{{ addslashes($menu->nama_menu ?? $menu->nama) }}', deskripsi: '{{ addslashes(str_replace(array("\r", "\n"), ' ', $menu->deskripsi)) }}', harga: {{ $menu->harga_jual ?? $menu->harga }}, foto: '{{ $menu->foto ? Storage::url($menu->foto) : '' }}', kategori: '{{ addslashes($kategoriNama) }}' })" class="menu-item {{ $filterGroup }} group flex flex-col cursor-pointer bg-white dark:bg-neutral-900/40 rounded-2xl overflow-hidden border border-neutral-100 dark:border-white/5 hover:border-primary/30 dark:hover:border-primary/50 hover:shadow-lg hover:shadow-primary/5 transition-all duration-300">
    <div class="h-40 w-full bg-neutral-50 dark:bg-neutral-800/50 flex items-center justify-center overflow-hidden">
        @if($menu->foto && Storage::disk('public')->exists($menu->foto))
            <img src="{{ Storage::url($menu->foto) }}" alt="{{ $menu->nama }}" class="w-full h-full object-cover transition-transform duration-500 group-hover:scale-105">
        @else
