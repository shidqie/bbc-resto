                ...
                <tbody class="divide-y divide-gray-50">
                    @forelse($pesanans as $p)
                        <tr class="hover:bg-gray-50/60 transition-colors group">
                            ...
                            <td class="px-4 py-3">
                                <p class="font-semibold text-gray-900 text-xs">{{ $p->detail_pesanan->first()->menu->nama_menu_paket ?? 'Paket Catering' }}</p>
                                <span class="inline-block text-xs font-semibold px-2 py-0.5 rounded-md bg-blue-50 text-blue-700 mt-1">
                                    {{ $p->detail_pesanan->first()->kuantitas }} Porsi
                                </span>
                            </td>
                            <td class="px-4 py-3 font-semibold text-gray-900 tabular-nums">
                                Rp {{ number_format($p->total_tagihan, 0, ',', '.') }}
                                @if($p->total_tagihan)
                                    <p class="text-xs text-gray-500 font-normal mt-0.5">DP: <span class="font-semibold text-blue-600">Rp {{ number_format($p->total_tagihan, 0, ',', '.') }}</span></p>
                                @endif
                            </td>
                            <td class="px-4 py-3">
                                @php
                            ...
                            <td class="px-4 py-3 text-center">
                                <div class="flex items-center justify-center gap-1.5">
                                    <a href="{{ route('admin.pesanan.catering.show', $p->id) }}" title="Detail" class="w-7 h-7 rounded-lg flex items-center justify-center bg-blue-50 text-blue-600 hover:bg-blue-100 transition-colors">
                                    ...
                                    <a href="{{ route('admin.pesanan.catering.pdf', $p->id) }}" target="_blank" title="Cetak" class="w-7 h-7 rounded-lg flex items-center justify-center bg-gray-100 text-gray-700 hover:bg-gray-200 transition-colors">
