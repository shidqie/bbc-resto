<x-layouts.landing>
    ...
    <section class="py-12 bg-canvas min-h-screen">
        <div class="max-w-2xl mx-auto px-4 lg:px-8">
            ...
            @if(session('success'))
                <div class="mb-6 p-4 bg-emerald-50 border border-emerald-200 rounded-3xl text-sm text-emerald-700 font-medium">
                    {{ session('success') }}
                </div>
            @endif

            {{-- Data Profil --}}
            <div class="bg-white rounded-[2.25rem] border border-primary/10 p-6 mb-6">
                ...
                <form method="POST" action="{{ route('konsumen.profile.update') }}" class="space-y-4">
                    ...
                    <div>
                        ...
                        <input id="nama" type="text" name="nama" value="{{ old('nama', $pelanggan->nama) }}"
                               ...
                               class="w-full px-4 py-3 bg-white border border-gray-200 rounded-3xl text-sm font-medium text-gray-900 placeholder-gray-300 transition-all duration-200 focus:border-primary">
                    ...
                    <div>
                        ...
                        <textarea id="alamat" name="alamat" rows="2" autocomplete="street-address"
                                  class="w-full px-4 py-3 bg-white border border-gray-200 rounded-3xl text-sm font-medium text-gray-900 placeholder-gray-300 transition-all duration-200 focus:border-primary">{{ old('alamat', $pelanggan->alamat) }}</textarea>
                        @error('alamat')<p class="text-xs text-red-500 font-medium mt-1.5">{{ $message }}</p>@enderror
                    </div>

                    <button type="submit"
                            class="w-full py-3.5 bg-primary hover:bg-primary-container text-white font-semibold text-sm rounded-3xl transition-all duration-200 active:scale-[0.99]">
                        Simpan Profil
                    </button>
                </form>
            </div>
            ...
            {{-- Ganti Password --}}
            <div class="bg-white rounded-[2.25rem] border border-primary/10 p-6">
                <h2 class="text-base font-bold text-primary mb-5 flex items-center gap-2">
                ...
                <form method="POST" action="{{ route('konsumen.profile.password') }}" class="space-y-4">
                    ...
                    @else
                        <p class="text-xs text-amber-600 font-medium bg-amber-50 border border-amber-200 rounded-3xl px-4 py-3">
