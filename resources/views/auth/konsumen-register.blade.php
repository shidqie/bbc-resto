        ...
        <div class="flex-1 flex items-center justify-center px-6 py-16 sm:px-12 bg-[#f5f5f0]">
            <div class="w-full max-w-[360px]">
                ...
                <form method="POST" action="{{ route('konsumen.register') }}" class="space-y-4" x-data="{ pw: '', pw2: '', get match() { return !this.pw2 || this.pw === this.pw2 } }">
                    ...
                    <div class="fu d2">
                        <label for="nama" class="block text-[11px] font-semibold text-gray-400 uppercase tracking-[0.1em] mb-1.5">Nama Lengkap</label>
                        <input id="nama" type="text" name="nama" value="{{ old('nama') }}"
                               required autofocus autocomplete="name"
                               placeholder="Nama Anda"
                               class="w-full px-4 py-3 bg-white border border-gray-200 rounded-3xl text-sm font-medium text-gray-900 placeholder-gray-300 transition-all duration-200 focus:border-[#0F2E23]">
                    </div>

                    <div class="fu d2">
                        <x-input-wa name="nomor_telepon" label="Nomor WhatsApp" :value="old('nomor_telepon')" :required="true" hint="Gunakan nomor yang dipakai saat memesan agar riwayat pesanan otomatis terhubung." />
                    </div>

                    <div class="fu d3">
                        <label for="email" class="block text-[11px] font-semibold text-gray-400 uppercase tracking-[0.1em] mb-1.5">Email <span class="text-gray-300 normal-case">(opsional)</span></label>
                    ...
                    <div class="fu d3">
                        ...
                        <input id="kata_sandi" type="password" name="kata_sandi" x-model="pw" required minlength="8" autocomplete="new-password"
                               ...
                               class="w-full px-4 py-3 bg-white border border-gray-200 rounded-3xl text-sm font-medium text-gray-900 placeholder-gray-300 transition-all duration-200 focus:border-[#0F2E23]">
                        <p class="text-[11px] text-gray-400 mt-1.5" x-show="pw.length > 0 && pw.length < 8">Minimal 8 karakter.</p>
                    </div>

                    <div class="fu d3">
                        <label for="kata_sandi_confirmation" class="block text-[11px] font-semibold text-gray-400 uppercase tracking-[0.1em] mb-1.5">Ulangi Password</label>
                        <input id="kata_sandi_confirmation" type="password" name="kata_sandi_confirmation" x-model="pw2" required autocomplete="new-password"
                               placeholder="••••••••"
                               class="w-full px-4 py-3 bg-white border rounded-3xl text-sm font-medium text-gray-900 placeholder-gray-300 transition-all duration-200 focus:border-[#0F2E23]"
                               :class="pw2 && !match ? 'border-red-300' : 'border-gray-200'">
                        <p class="text-[11px] text-red-500 font-medium mt-1.5" x-show="pw2 && !match" x-cloak>Password tidak cocok.</p>
                    </div>
                ...
                <p class="text-center text-sm text-gray-500 font-medium mt-6 fu d4">
                    Sudah punya akun?
                    <a href="{{ route('konsumen.login') }}" class="text-[#0F2E23] font-bold hover:opacity-70 transition-opacity">Masuk</a>
                </p>
