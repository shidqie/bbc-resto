    ...
    <div class="w-full p-6 space-y-5">
        ...
        <div class="bg-white rounded-xl border border-gray-200 px-5 py-4">
            <form action="{{ route('laporan.stok') }}" method="GET" class="flex flex-col md:flex-row gap-3 items-end flex-wrap" x-data="{ jenis: '{{ $jenisPersediaan }}' }">
                ...
                <div class="flex-1 min-w-[130px]">
                    <label class="block text-sm font-semibold text-gray-500 mb-1">Jenis Persediaan</label>
                ...
                <div class="flex-1 min-w-[130px]">
                    <label class="block text-sm font-semibold text-gray-500 mb-1">Kategori Bahan</label>
                    <select name="kategori_id" class="w-full px-3 py-2 text-sm border border-gray-200 rounded-lg bg-white outline-none focus:ring-1 focus:ring-blue-300 transition-all">
                ...
                <div class="flex-1 min-w-[130px]">
                    <label class="block text-sm font-semibold text-gray-500 mb-1">Bahan Baku</label>
                    <select name="bahan_baku_id" class="w-full px-3 py-2 text-sm border border-gray-200 rounded-lg bg-white outline-none focus:ring-1 focus:ring-blue-300 transition-all">
            ...
            <table class="w-full text-sm">
                ...
                <tbody class="divide-y divide-gray-50">
                    ...
                    <tr class="hover:bg-gray-50/60 transition-colors">
                        ...
                        <td class="px-4 py-3 font-mono text-sm font-semibold text-gray-500">{{ $bahan->kode_bahan ?? '-' }}</td>
                        <td class="px-4 py-3">
                        ...
                        <td class="px-4 py-3 text-right text-sm font-semibold text-gray-700">{{ number_format($item['stok_awal'], 2) }}</td>
                        <td class="px-4 py-3 text-right text-sm font-semibold text-emerald-600">+{{ number_format($item['stok_masuk'], 2) }}</td>
                        <td class="px-4 py-3 text-right text-sm font-semibold text-red-600">-{{ number_format($item['stok_keluar'], 2) }}</td>
                        ...
                        <td class="px-4 py-3 text-sm text-gray-500">{{ $bahan->satuan?->singkatan ?? '-' }}</td>
                        <td class="px-4 py-3">
