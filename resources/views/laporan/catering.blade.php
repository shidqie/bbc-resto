...
<div class="flex-1 overflow-auto bg-gray-50/50 text-gray-800 font-sans">
    <div class="p-4 md:p-6 lg:p-8 max-w-[1200px] mx-auto space-y-6">
        ...
        <x-ui.page-header title="Laporan Pendapatan Catering" subtitle="Ringkasan transaksi catering yang telah selesai">
            <x-slot:actions>
                <x-ui.button href="{{ route('laporan.catering.cetak', ['start_date' => $startDate, 'end_date' => $endDate]) }}" icon="fa-print" variant="outline">Cetak PDF</x-ui.button>
            </x-slot:actions>
        </x-ui.page-header>

        <form action="{{ route('laporan.catering') }}" method="GET" class="bg-white p-4 rounded-2xl shadow-sm border border-gray-100 flex flex-wrap gap-4 items-end">
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
            <x-ui.stat-card label="Total Pendapatan (Catering)" value="Rp {{ number_format($totalPendapatan, 0, ',', '.') }}" icon="fa-wallet" color="green" />
            ...
            <table class="w-full text-left border-collapse min-w-[800px]">
                ...
                <tbody class="divide-y divide-gray-50 text-sm">
                    ...
                    @empty
                        <tr>
                            <td colspan="5">
                                <x-ui.empty-state icon="fa-folder-open" title="Tidak ada data catering" />
                            </td>
                        </tr>
                    @endforelse
                </tbody>
