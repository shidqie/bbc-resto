...
<div class="flex-1 overflow-auto bg-gray-50/50 text-gray-800 font-sans">
    <div class="w-full p-6 ] space-y-6">
        ...
        <x-ui.page-header title="Laporan Pendapatan Nasi Box" subtitle="Ringkasan transaksi nasi box yang telah selesai">
            <x-slot:actions>
                <x-ui.button href="{{ route('laporan.nasibox.cetak', ['start_date' => $startDate, 'end_date' => $endDate]) }}" icon="printer" variant="outline">Cetak PDF</x-ui.button>
            </x-slot:actions>
        </x-ui.page-header>
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
            <x-ui.stat-card label="Total Pendapatan (Nasi Box)" value="Rp {{ number_format($totalPendapatan, 0, ',', '.') }}" icon="wallet" color="green" />
            ...
            <table class="w-full text-left border-collapse min-w-[800px]">
                ...
                <tbody class="divide-y divide-gray-50 text-sm">
                    ...
                    @empty
                        <tr>
                            <td colspan="5">
                                <x-ui.empty-state icon="sparkles" title="Tidak ada data nasi box" />
                            </td>
                        </tr>
                    @endforelse
                </tbody>
