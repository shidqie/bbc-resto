...
<div class="flex-1 overflow-auto bg-gray-50/50 text-gray-800 font-sans">
    <div class="p-4 md:p-6 lg:p-8 max-w-[1200px] mx-auto space-y-6">
        ...
        <x-ui.page-header title="Laporan Pendapatan Nasi Box" subtitle="Ringkasan transaksi nasi box yang telah selesai">
            <x-slot:actions>
                <x-ui.button href="{{ route('laporan.nasibox.cetak', ['start_date' => $startDate, 'end_date' => $endDate]) }}" icon="fa-print" variant="outline">Cetak PDF</x-ui.button>
            </x-slot:actions>
        </x-ui.page-header>

        <form action="{{ route('laporan.nasibox') }}" method="GET" class="bg-white p-4 rounded-2xl shadow-sm border border-gray-100 flex flex-wrap gap-4 items-end">
            ...
            <x-ui.button type="submit" icon="fa-filter">Filter Data</x-ui.button>
        </form>

        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
            <x-ui.stat-card label="Total Pendapatan (Nasi Box)" value="Rp {{ number_format($totalPendapatan, 0, ',', '.') }}" icon="fa-wallet" color="green" />
            <x-ui.stat-card label="Total Transaksi Selesai" :value="$totalTransaksi" icon="fa-chart-line" color="blue" />
        </div>
            ...
            <table class="w-full text-left border-collapse min-w-[800px]">
                <thead>
                    <tr class="bg-white text-gray-500 text-xs uppercase tracking-wider border-b border-gray-100">
                        ...
                        <th class="px-6 py-4 font-semibold">Paket Nasi Box</th>
                ...
                <tbody class="divide-y divide-gray-50 text-sm">
                    @forelse($pesanans as $p)
                        <tr class="hover:bg-gray-50/50 transition-colors">
                            ...
                            <td class="px-6 py-4">
                                <span class="text-[#F97316]">{{ $p->menu->nama ?? 'Nasi Box' }}</span> ({{ $p->jumlah_box }} Box)
                            </td>
                    ...
                    @empty
                        <tr>
                            <td colspan="5">
                                <x-ui.empty-state icon="fa-folder-open" title="Tidak ada data nasi box" />
                            </td>
                        </tr>
                    @endforelse
                </tbody>
