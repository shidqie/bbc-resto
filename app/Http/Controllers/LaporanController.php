...
class LaporanController extends Controller
{
    public function penjualan(Request $request)
    {
        ...
        if ($jenisPenjualan === 'dinein')       $query->where('jenis_pesanan_id', 1);
        elseif ($jenisPenjualan === 'catering') $query->where('jenis_pesanan_id', 2);
        elseif ($jenisPenjualan === 'nasibox')  $query->where('jenis_pesanan_id', 3);
    ...
    public function stok(Request $request)
    {
        ...
        if ($kategoriId)  $queryBahan->where('kategori_bahan_baku_id', $kategoriId);
        if ($bahanBakuId) $queryBahan->where('id', $bahanBakuId);
        ...
        $laporanBahan = $queryBahan->orderBy('nama_bahan')->get()->map(function($bahan) use ($startDate, $endDate, $jenisPersediaan) {
            ...
            if ($jenisPersediaan) $qMutasi->where('jenis_stok', strtoupper($jenisPersediaan));
    ...
    public function pengadaan(Request $request)
    {
        ...
        $topBahan = \App\Models\DetailPengadaanBahan::with('bahan_baku')
